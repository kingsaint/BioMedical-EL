...
concepts = [Concept(id = "C1",info={"tst_attr":3}),
...
basic_implementation = Basic_Lexical_Knowledge_Base(small_example)
rdf_implementation = RDF_Lexical_Knowledge_Base(small_example)
def get_basic_implementation():
    return basic_implementation
def get_rdf_implementation():
    return rdf_implementation
@pytest.fixture(params=[get_basic_implementation,get_rdf_implementation])
def LKB_Implementation(request):
    return request.param
def test_concept_getting(LKB_Implementation):
    lkb = LKB_Implementation()
...
def test_forward_edge_getting(LKB_Implementation):
    correct_result = sorted([(Conceptual_Relation(id="R1",string="Test Relation 1"),Concept(id = "C2",info={"tst_attr":4})),
                             (Conceptual_Relation(id="R2",string="Test Relation 2"),Concept(id = "C3",info={"tst_attr":5}))
                             ],
...
def test_get_terms_for_concept(LKB_Implementation):
    correct_result = sorted([Term(id="T2",string="Test Term 2"),
                             Term(id="T3",string="Test Term 3")
                             ],
                            key = (lambda term: term.id))
    result = sorted(LKB_Implementation().get_terms_from_concept_id("C3"),key = (lambda term: term.id))
    assert result == correct_result
