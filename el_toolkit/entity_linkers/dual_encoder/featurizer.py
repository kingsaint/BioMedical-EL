...
class DualEncoderFeaturizer:
    def __init__(self,dual_encoder,hvd=None):
...
class DualEncoderTrainFeaturizer(DualEncoderFeaturizer):
    def __init__(self,*args,lkb,num_hard_negatives=0,num_random_negatives=0,num_max_mentions=8,**kwargs):
        ...
        if num_hard_negatives:
            self._encoded_concepts = self._dual_encoder.concept_embedder._concept_encoder.from_lkb(lkb,self._hvd)
            self._embeddings = self._dual_encoder.concept_embedder.embed_from_concept_encodings(self._encoded_concepts,self._hvd)
    def featurize_doc(self,doc):
        num_mentions = len(doc.mentions)
        if self._lower():
                doc = doc.lower()
        doc_token_ids,doc_token_mask,mention_start_indices,mention_end_indices,seq_tag_ids,too_long  = self._dual_encoder.document_embedder.encode_document(self,doc)
        ...
        for candidate_id_list in candidate_id_lists:
            for candidate_id in candidate_id_list:
                if self._encoded_concepts:
                    ...
                    encoded_candidate = self._dual_encoder.concept_embedder.encode_concept(candidate_id)
...
class EvalFeaturizer(DualEncoderFeaturizer):
    def featurize_doc(self,doc):
        num_mentions = len(doc.mentions)
        if self._lower():
                doc = doc.lower()
